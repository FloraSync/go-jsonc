...
> Historical qualification record: this packet closed the original Go 1.26
> matrix. The v1.0.0 release candidate later added current Go 1.27 on Linux,
> macOS, and Windows, plus stable `encoding/json/v2` interoperation and the
> documented `GOEXPERIMENT=nojsonv2` compatibility control. The current
> workflow and compatibility contract supersede Go-1.26-only statements below.
